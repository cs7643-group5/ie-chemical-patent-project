...
if __name__ == '__main__':
    ...
    model_re = RelationClassifier(bert_model).to(device)
    ...
    model_ner.load_state_dict(torch.load("saved_models/pytorch_model.bin", map_location=device))

    _, i2arg = ee_preprocessor.map2ind()
    # trigger2i, i2trigger = preprocessor.map2ind(tags)
    # tag2i, i2tag = preprocessor.map2ind(tags)
